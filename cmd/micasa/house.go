...
type houseCmd struct {
	Show   houseShowCmd   `cmd:"" default:"withargs" help:"Display house profile."`
	Update houseUpdateCmd `cmd:""                    help:"Update house profile (creates if none exists)."`
}

// houseProfileJSON is the JSON representation of a house profile.
...
func (cmd *houseShowCmd) Run() error {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return fmt.Errorf("no house profile exists -- create one with: micasa house update")
	}
	if err != nil {
...
type houseUpdateCmd struct {
	Nickname         *string  `help:"House nickname."                      name:"nickname"`
	Address          *string  `help:"Street address (line 1)."             name:"address"`
	AddressLine2     *string  `help:"Address line 2."                      name:"address-line-2"`
	City             *string  `help:"City."                                name:"city"`
	State            *string  `help:"State or province."                   name:"state"`
	PostalCode       *string  `help:"Postal/ZIP code."                     name:"postal-code"`
	YearBuilt        *int     `help:"Year the house was built."            name:"year-built"`
	Sqft             *int     `help:"Interior square footage."             name:"sqft"`
	LotSqft          *int     `help:"Lot square footage."                  name:"lot-sqft"`
	Bedrooms         *int     `help:"Number of bedrooms."                  name:"bedrooms"`
	Bathrooms        *float64 `help:"Number of bathrooms (e.g. 2.5)."     name:"bathrooms"`
	...
	ExteriorType     *string  `help:"Exterior material."                  name:"exterior-type"`
	HeatingType      *string  `help:"Heating system type."                name:"heating-type"`
	CoolingType      *string  `help:"Cooling system type."                name:"cooling-type"`
	...
	InsuranceCarrier *string  `help:"Insurance carrier name."             name:"insurance-carrier"`
	...
	PropertyTaxCents *int64   `help:"Annual property tax in cents."        name:"property-tax-cents"`
	HOAName          *string  `help:"HOA name."                           name:"hoa-name"`
	HOAFeeCents      *int64   `help:"Monthly HOA fee in cents."           name:"hoa-fee-cents"`
	DBPath           string   `help:"SQLite database path."               env:"MICASA_DB_PATH"`
}

func (cmd *houseUpdateCmd) Run() error {
	if cmd.InsuranceRenewal != nil {
		if _, err := time.Parse(time.DateOnly, *cmd.InsuranceRenewal); err != nil {
			return fmt.Errorf("invalid insurance renewal date %q: expected YYYY-MM-DD", *cmd.InsuranceRenewal)
		}
	}
...
func (cmd *houseUpdateCmd) applyTo(p *data.HouseProfile) {
	if cmd.Nickname != nil {
	...
	if cmd.InsuranceRenewal != nil {
		t, _ := time.Parse(time.DateOnly, *cmd.InsuranceRenewal)
		p.InsuranceRenewal = &t
	}
